...
import { MAX_GOAL_PROFILE_EMAILS, MAX_IMPORTANCE_INBOX_EMAILS, MAX_IMPORTANCE_SCAN_EMAILS, type GoalsProfile, readEffectiveImportanceRules, readGoalsProfile, readRankedInboxCache, saveGoalsProfile, saveRankedInboxCache, scoreEmails, validateGoalsProfile } from "@/lib/importance";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const profile = await withFileLock(accountLockPath(session.email, "goals_profile.lock"), async () => {
      ...
      if (current && body.refresh !== true && (await readRankedInboxCache(session.email))?.schema_version === 2) return current;
      ...
      if (!stored || body.refresh === true) {
        ...
        stored = { owner_email: session.email, generated_at: new Date().toISOString(), sampled_emails: emails.length, profile: goalsProfile };
      ...
      const triaged = await triageEmailsWithModel(inboxCandidates, `onboarding-triage-${Date.now()}`);
